...
defaultPath = 'C:\\Users\ChrisGomes\Projects\MediaStock\media0000000000{0:02d}.csv' # base path for all media csvs
media = pd.read_csv(defaultPath.format(0)) # add first media csv

for i in range(1, 16):
    media = media.append(pd.read_csv(defaultPath.format(i)), ignore_index=True) # append each csv numbered after 0

companies = pd.read_csv('C:\\Users\ChrisGomes\Projects\MediaStock\companyNamesFormatted.csv') # Source: https://www.nasdaq.com/screening/company-list.aspx
